...
export class ScanService {
  ...
  progress$ = combineLatest({
    ...
    total: this.watchingCount$,
  }).pipe(map((state) => state.scanned / state.total));
  progressCompleted$ = combineLatest({
    scanned: this.scannedCount$,
    total: this.watchingCount$,
  }).pipe(
    filter(
      (state) =>
        !!state.scanned && !!state.total && state.scanned === state.total,
    ),
  );
  ...
  async startScan(force?: boolean) {
    ...
    this.subs.add(
      watcher.state.subscribe((state) => {
        if (
          !state.latest?.endsWith('.png') &&
          !state.latest?.endsWith('.jpg') &&
          !state.latest?.endsWith('.jpeg')
        )
          return;
        if (
          state.latest &&
    ...
    this.subs.add(
      this.itemsUpdated$.pipe(debounceTime(1000)).subscribe(() => {
